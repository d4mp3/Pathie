...
class RouteListAPIViewTests(TestCase):
    ...
    def test_list_routes_success_default_parameters(self):
        ...
        self.assertEqual(route_ids, [self.route3_saved.id, self.route2_saved.id, self.route1_saved.id])
    ...
    def test_list_routes_ordering_by_name_ascending(self):
        ...
        self.assertEqual(route_names, ["Saved Route 1", "Saved Route 2", "Saved Route 3"])
    ...
    def test_list_routes_ordering_by_name_descending(self):
        ...
        # Verify ordering
        route_names = [r["name"] for r in response.data["results"]]
        self.assertEqual(route_names, ["Saved Route 3", "Saved Route 2", "Saved Route 1"])

    def test_list_routes_ordering_by_points_count_descending(self):
        """Test ordering routes by points count in descending order."""
        self.client.credentials(HTTP_AUTHORIZATION=f"Token {self.token1.key}")

        response = self.client.get(f"{self.route_list_url}?status=saved&ordering=-points_count")
    ...
    def test_list_routes_points_count_excludes_removed_points(self):
        ...
        route2_data = next(r for r in response.data["results"] if r["id"] == self.route2_saved.id)
...
class TagListAPIViewTests(TestCase):
    ...
    def test_list_tags_excludes_inactive(self):
        ...
        self.assertTrue(Tag.objects.filter(name="Inactive Tag", is_active=False).exists())
    ...
    def test_list_tags_with_null_description(self):
        ...
        no_desc_tag = next((t for t in response.data if t["name"] == "No Description"), None)
